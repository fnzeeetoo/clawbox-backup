...
export class StorageManager {
  ...
  async detectUSBDevices() {
    ...
    try {
      const { stdout } = await execAsync('lsblk -J -o NAME,LABEL,SIZE,TYPE,MOUNTPOINT,TRAN');
      const data = JSON.parse(stdout);

      for (const device of data.blockdevices) {
        // Only include actual USB devices (TRAN=usb) or explicitly removable media
        const isUSB = device.tran === 'usb' || device.name.startsWith('sd') && !device.name.includes('nvme');
        if (!isUSB) continue;
